    ...
    methods: {
        ...
        ChangeDropdown: function () {
            if (app.Selected !== null) {
                $.ajax({
                    url: "/api/ApplicationApi/GetApplicationDropdownOptionsByHeaderId",
        ...
        AddEditOption: function () {
            ...
            $.ajax({
                url: "/api/ApplicationApi/CheckTitleAndValueExists",
                ...
                if (response.Item1 === false && response.Item2 === false) {
                    $.ajax({
                        url: "/api/ApplicationApi/InsertUpdateOptionRecord",
                        data: JSON.stringify(data),
                        type: "Post",
                        contentType: "application/json",
                        dataType: "json"
        ...
        Delete: function (optionId) {
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: "/api/ApplicationApi/DeleteDropdownOptionByOptionId",
                    data: { id: optionId },
                    type: "GET",
                    contentType: "application/json",
                    dataType: "json"
        ...
        addHeader: function () {
            $("#kt_modal_template_Header").modal("show");
        },
        SubmitHeader: function () {
            if (this.headerText == '') {
                toastr.error("Plase add header", 'Warning');
                return ;
            }

            $.ajax({
                url: "/api/ApplicationApi/CheckHeader",
                data: { text: this.headerText },
                type: "Get",
                contentType: "application/json",
                dataType: "json"
            }).done(function (response) {
                if (response === false) {
                    $.ajax({
                        url: "/api/ApplicationApi/AddHeader",
                        data: { text: app.headerText },
                        type: "Get",
                        contentType: "application/json",
                        dataType: "json"
                    }).done(function (response) {
                        toastr.success("Header Added", 'Added');
                        app.headerText = '';
                        $("#kt_modal_template_Header").modal("hide");

                    })
                }
                if (response === true) {
                    toastr.error("header already exist", 'Warning');
                    return;
                }
            })
